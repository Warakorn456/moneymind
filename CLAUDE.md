...
| AI | Maya agent สั่งงานได้: บันทึกรายการ/เพิ่มออม/จ่ายหนี้/ตั้งงบ (tools ทำงานกับ DB ใน browser, ใช้ apiKey ของ user, Gemini OpenAI-compat endpoint) |
...
| `hermes_agent.py` | `hermes_agent.py` | ✅ deploy แล้ว — **AI agent (tool calling)** AI chat ใน bot; entry point `hermes_agent.chat(text)`; lazy-import `stock_bot` กัน circular; **provider สลับได้ (default gemini-2.5-flash, OpenRouter fallback)** |
...
| `_detect_cat_ai(desc)` | AI จัดหมวด (Phase 2) — `hermes_agent.simple_complete()`, cache ใน `cat_cache.json`, fallback None เมื่อ 429/error |
...
- **Hermes agent (tool calling) เพิ่ม 2026-06-15** — `hermes_agent.py` ใหม่: AI chat แบบ tool calling ผ่าน OpenRouter (`chat()` entry point), ห่อ 12 function เดิมใน stock_bot เป็น tools (get_summary/budget/debt/saving/portfolio/prices/add_transaction/set_target/dca ฯลฯ), agent loop เรียกได้หลาย tool ต่อคำถาม; `stock_bot.py` เพิ่ม `import hermes_agent` (optional, try/except) + AI-chat fallback ใน else-block (เฉพาะ `chat_id == CHAT_ID`, text ≥ 4 ตัว, ใช้ข้อความต้นฉบับไม่ใช่ `.lower()`); secrets ผ่าน `mm_secrets.OPENROUTER_KEY` / `HERMES_MODEL`
