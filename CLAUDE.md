...
**สิ่งที่เหลือทำก่อนเปิดใช้จริง:** (1) รอ Omise KYC ผ่าน (2) เขียนโค้ด checkout (สร้าง charge/token ฝั่ง client ด้วย public key) + webhook รับผล (คล้าย pattern `ai_proxy.py`/`caddy`, ต้องเป็น long-running service มี TLS) → ต่อกับ flow ตั้ง `subscriptionActive`/`subscriptionExpiry` อัตโนมัติ (ตอนนี้ตั้งได้แค่ manual ผ่าน `grant_subscription.py`) — เขียน/ทดสอบด้วย test-mode key คู่ขนานระหว่างรอ KYC ได้เลย
