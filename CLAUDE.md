...
| Travel 3D Globe | **ลูกโลก 3 มิติหมุนได้** บนหน้า travel — `globe.gl@2.33.2` (Three.js bundled, lazy-load ผ่าน `_loadScript` ตอนเปิดหน้า) + texture **earth-blue-marble** (1.46MB res สูง — เดิม earth-night แตกตอนซูม) จาก unpkg three-globe; พิมพ์ประเทศ/เมือง → `_geocode()` (Nominatim ฟรี ไม่ต้อง key, cache ใน `DB.travel.places`) → `_flyToPlace()` หมุนลูกโลกไปหา (`pointOfView`) → ปุ่ม **✓ เคยไปแล้ว / ★ อยากไป** (`travelMark`); หมุดเขียว=เคยไป เหลือง=อยากไป; auto-rotate toggle (`travelToggleSpin`); `controls().minDistance=160/maxDistance=520` จำกัดซูมกันภาพแตก; `_syncTravelPlacesFromTx()` geocode สถานที่จากรายการ tx อัตโนมัติ (ทีละ 1.1s กัน rate-limit) + migration `geoVer=2` ล้างพิกัดเก่าที่ผิด; **globe อยู่ใน card ระดับ page (persistent) แยกจาก `#travel-content` ที่ re-render** เพื่อไม่ทำลาย WebGL context; ข้อมูลเก็บใน `DB.travel.places=[{name,lat,lng,visited,label,fromTx}]` (sync localStorage+Firestore) |
| Travel Globe gotcha | **Nominatim geocode ต้องเรียง `importance` แล้วเลือกตัวสูงสุด** — ค้นหา "ญี่ปุ่น" (accept-language=th) ผลแรกคือ "บ้านญี่ปุ่น อยุธยา ประเทศไทย" (importance 0.147) ไม่ใช่ประเทศญี่ปุ่น (0.938); fix: `limit=8` + `sort((a,b)=>b.importance-a.importance)[0]`. **ค้นหาใน UI ใช้ `_geocode(q,true)` (fresh) เสมอ** เลี่ยง cache พิกัดผิดเก่า + อัปเดตหมุดเดิมที่ผิดให้ถูกทันที |
| Travel Globe zoom เมือง | ซูมลึกถึงระดับเมือง/จังหวัด — `minDistance=108` (เดิม 160 แน่นไป); **เส้นขอบประเทศ vector** จาก `globe.gl/example/datasets/ne_110m_admin_0_countries.geojson` (`polygonsData`, cap โปร่งใส stroke ขาวจาง) คมชัดทุกระดับซูม (แก้ปัญหา texture แตกตอนซูมลึก); `_geocode` คืน `addresstype` → `_altForType()` กำหนดระยะ fly: country=1.25, state/province=0.55, city=0.32, town/village=0.2 → ค้น "ญี่ปุ่น" zoom ไกลเห็นทั้งประเทศ, ค้น "โอซาก้า" zoom ใกล้ระดับเมือง; เก็บ `type` ใน `DB.travel.places` |
