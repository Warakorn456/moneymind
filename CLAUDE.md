...
- **🔧 cron_health.py — ขยาย window + recovery detection (fix 2026-06-18)** — `check_crash` เดิมอ่าน last 15 lines → traceback ยาว (urllib stack ~20 บรรทัด) ทำให้ header 'Traceback' หลุดออกนอก window → **พลาด crash จริง** (เคสจริง body_parser); แก้: window 15→**45** + ตรวจ recovery (ถ้ามี log line ขึ้นต้นด้วยวันที่ YYYY-MM-DD หลัง Traceback ล่าสุด = รันใหม่ผ่านแล้ว ไม่ alert); ต้อง `import re`
