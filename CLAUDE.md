...
- **🐛 หุ้นที่ชื่นชอบดึงราคาไม่ได้เป็นแถว (เจอ+แก้ 2026-08-04)** — user รายงานพร้อม screenshot: บัญชีจริงมีหุ้นในลิสต์ ~19 ตัว ส่วนใหญ่ขึ้น "ดึงราคาไม่ได้" สีแดง มีแค่ไม่กี่ตัวสำเร็จ (NVDA/PLTR/FLY). สาเหตุ: `getPriceFor()`→`yhFetch()`→`proxyFetch()` (index.html ~6283) พึ่ง **free public CORS proxy 4 เจ้า** (allorigins.win/corsproxy.io/codetabs.com/thingproxy) เพราะเบราว์เซอร์เรียก Yahoo Finance ตรงไม่ได้ (CORS) — proxy ฟรีพวกนี้มี rate-limit ต่ำ ส่วน `_loadFavStockPrices()` เดิมยิง request ให้ทุกหุ้นรัวๆ ไม่มีหน่วงเวลาเลย (`for...of` + `await` ต่อกันไม่มี delay) พอมีหุ้นเยอะ (~19 ตัว) ก็ชน rate-limit ของ proxy เกือบหมด. **แก้ 3 จุด:** (1) เพิ่ม cache ราคาไว้ 90 วิ (`_favStockPriceCache`) กัน re-fetch ซ้ำทุกครั้งที่ `renderInv` ทำงาน (tab switch/auto-refresh ทุก 2 นาที/หลังบันทึก — ก่อนหน้านี้ยิงซ้ำทุกรอบ) (2) หน่วง 400ms ระหว่างคิวหุ้นแต่ละตัว (`await new Promise(...)`) ลด burst rate ชน proxy เดียวกับ pattern ที่ใช้กับ Nominatim ใน Travel feature (3) การ์ดที่ fail ตอนนี้กดที่ข้อความ "ดึงราคาไม่ได้" เพื่อลองใหม่เฉพาะตัวได้ (`_retryFavStockPrice`) ไม่ต้องรีเฟรชทั้งหน้า. ทดสอบยืนยัน cache ทำงาน (สลับแท็บแล้วราคาไม่กระพริบ/ไม่ยิง fetch ซ้ำ). **ข้อจำกัดที่ยังแก้ไม่ได้ 100%:** proxy ฟรีพวกนี้เป็นบุคคลที่สามที่ไม่เกี่ยวกับโปรเจกต์ ไม่การันตี uptime/quota — ถ้ายังพังบ่อยหลังแก้นี้ ต้องพิจารณาเปลี่ยนไปใช้ backend ของเราเอง (เช่น proxy ผ่าน `ai_proxy.py`/VM ที่มีอยู่แล้ว) แทนพึ่ง third-party ฟรีทั้งหมด — เป็นปัญหาเดียวกับหน้า Investments ปกติที่ใช้ `getPriceFor()`/`refreshAllInvPrices()` เดิมด้วย (แค่ไม่เคยมีคนรายงานเพราะพอร์ตปกติมีไม่กี่ตัว) ไม่ใช่บั๊กที่เกิดจากฟีเจอร์ใหม่นี้โดยตรง
