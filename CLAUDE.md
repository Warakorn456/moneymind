...
**สถานะ:** infra + client wiring + Gemini proxy key พร้อมหมดแล้ว **ยังไม่เปิดใช้งานจริง** เพราะยังไม่มีระบบรับเงิน (สมัคร Omise/Opn Payments แล้ว อยู่ระหว่าง KYC — มี test-mode key ใช้พัฒนาได้แล้ว ยังไม่มีโค้ด checkout/webhook)
...
| Omise/Opn Payments (เพิ่ม 2026-07-03) | สมัครแล้ว บัญชี `warakornbest6@gmail.com` — dashboard.omise.co, อยู่ระหว่าง KYC (บุคคลธรรมดา) รอ Opn ตรวจเอกสาร 2-3 วันทำการ. **Test-mode key ใช้ได้แล้วตอนนี้** (`mm_secrets.OMISE_PUBLIC_KEY`/`OMISE_SECRET_KEY` ← `.env` MM_OMISE_PUBLIC_KEY/MM_OMISE_SECRET_KEY, prefix `pkey_test_`/`skey_test_`) ใส่ทั้งโน้ตบุ๊ก+VM แล้ว — พอ KYC ผ่านจะได้ live key (`pkey_live_`/`skey_live_`) มาสลับแทน |
