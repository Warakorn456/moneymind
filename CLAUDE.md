...
| ~~รายการซ้ำ~~ | ❌ **ถอดหน้า/UI ออก 2026-06-26** (`renderRecurring`/modal `m-recur`/`openRecurModal`/`saveRecur` ลบแล้ว) — เหลือ `applyRecurringTx()` auto-apply กับ `DB.recurringTx` เดิม + ปฏิทิน/Dashboard ยังอ่านข้อมูล |
| ~~จัดลำดับ (Priority)~~ | ❌ **ถอดออก 2026-06-26** (`renderPriority`/`PQ_QUADS`/`getPriorities`/ฯลฯ + CSS `.pq-*` ลบแล้ว; `DB.priorities` ยังอยู่ใน DB ไม่มี UI) |
