...
| Travel Journal (บันทึกทริป) | **เพิ่ม 2026-08-03** — สมุดบันทึกการเดินทางแยกจาก pin เดิม เก็บ `DB.travel.tripLogs=[{id,place,province,country,startDate,endDate,desc,visited}]`; ปุ่ม "เพิ่มบันทึกทริป" เปิด modal (`openTripLogModal`/`saveTripLog`/`deleteTripLog`) กรอกประเทศ/จังหวัด/สถานที่/วันที่ไป-กลับ/สถานะ(เคยไป-อยากไป)/คำบรรยายทริปเป็นข้อความยาวได้; การ์ดแยก 2 กลุ่ม (เคยไปแล้ว เรียงวันที่ล่าสุดก่อน / อยากไป) คลิกชื่อสถานที่ = `travelFly()`; ปุ่ม "✓ เคยไปแล้ว" บนการ์ดอยากไปเรียก `markTripLogVisited()`; หลัง save เรียก `_tripLogSyncPin()` (async) upsert เข้า `DB.travel.places` เดิมด้วย (geocode ผ่าน `_geocode` ถ้ายังไม่มี pin) แล้ว **re-render อีกครั้งหลัง sync resolve** (`.then(renderTravel)`) กัน pin-chip ส่วนเก่าค้างสถานะเก่า (เจอบั๊กนี้ตอนทดสอบ — ถ้า render ครั้งเดียวก่อน sync จะมีช่วงที่ journal card อัปเดตแล้วแต่ chip เดิมยังไม่ตาม); โมดัลสร้างแบบ dynamic-inject (`document.body.appendChild`, แบบเดียวกับ feedback modal) ใช้ class `.overlay`/`.modal`/`.finput` เดิมเพื่อ theme ตามแอปอัตโนมัติ (ไม่ hardcode สีเหมือน feedback modal); เพิ่ม i18n keys `travel_journal_*` (21 keys) เข้า `I18N.th`/`I18N.en` |
