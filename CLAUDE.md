...
| I18N (กำลังทำ, เริ่ม 2026-07-02) | **ระบบแปลภาษา TH/EN** — `I18N={th:{},en:{}}` (500+ keys, กุญแจต้องเท่ากันทั้ง 2 ภาษาเสมอ — เช็คด้วย `Object.keys(I18N.th).length===Object.keys(I18N.en).length`) + **`tr(key)`** (⚠️ ไม่ใช่ `t()` — ชื่อนี้ชนกับตัวแปร loop `t`/`tx` ที่ใช้ทั่วไฟล์แทน transaction object ทำให้เกิด `TypeError` ถ้าตั้งชื่อ `t`) + `setLang()`/`toggleLang()` + `applyLang()`; เก็บที่ `localStorage.mm_lang` (ใช้ได้ก่อน login) + sync เข้า `DB.settings.lang` หลัง login; ปุ่มสลับภาษา `#lang-btn` อยู่ข้าง theme toggle; attribute รองรับ: `data-i18n` (textContent), `data-i18n-ph` (placeholder), `data-i18n-html` (innerHTML), `data-i18n-title` (title tooltip); `PAGE_TITLE` เป็น `Proxy` route ผ่าน `tr(page)`; ฟังก์ชันช่วย bilingual: `catLabel(c)` (หมวดหมู่ ใช้แทน `c.label` ตรงๆ ทุกที่ที่ iterate INC_CATS/EXP_CATS ดิบ), `getCat()` (คืน labelEn อัตโนมัติถ้า currentLang==='en'), `monthShortArr()`/`monthFullArr()`/`beYear()` (เดือน/ปี), `insTypeLabel()`/`insInsuredLabel()`/`insCycleLabel()` (ประกัน); **หน้าที่แปลครบแล้ว:** Login/Register/Forgot, Settings, Dashboard (ทุก widget), Transactions (list/filter/modal/scan ใบเสร็จ/bank statement import), Savings, Banks (+โอนเงิน), Debts (+payoff planner), Insurance, Investments (โครงหลัก — modal เพิ่ม/แก้ไข, พอร์ต, กราฟ), Tax (ฟอร์มลดหย่อน+ผลคำนวณ), Balance/Plan (งบดุล+ตาราง 80 ปี+modal), Retire (ฟอร์ม+ผลลัพธ์+กราฟ), Profile (ข้อมูลส่วนตัว+การศึกษา+งาน+ทักษะ), Calendar (+แก้ bug `CAL_MON_FULL`/`CAL_MON_SHORT` เป็น stale var ไม่ re-evaluate ตอนสลับภาษา → เปลี่ยนเป็นเรียก `monthFullArr()`/`monthShortArr()` สดทุกครั้ง), Travel (แผนที่+สรุปทริป+ประมาณงบ), Subscriptions, OT Calculator, Loan Calculator (+แก้ bug `r.beYear(date...)` ผิด syntax เป็น `beYear(r.date...)` ในตาราง amortization); **ยังไม่แปล:** Admin, AI Usage, AI Maya chat responses, toast messages ทั่วไฟล์ที่เหลือ (รวม Tax Optimizer sub-feature ในหน้า Tax และ Rebalancing panel ในหน้า Investments ที่ข้ามไปยังไม่แปล); เพิ่ม element ใหม่ต้องเติม `data-i18n="key"` + เพิ่ม key เข้า `I18N.th`/`I18N.en` คู่กันเสมอ (ห้ามลืมฝั่งใดฝั่งหนึ่ง); **gotcha เดียวกับ push token:** โค้ดที่ต้องรันหลัง login ทั้ง fresh-login และ session-restore ห้าม patch `checkLogin` (สายเกินไป) ต้องเช็ค `currentUser` ตรงๆ |
