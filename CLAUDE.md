...
| `/v1/financials` (เพิ่ม 2026-08-04 ค่ำ, ไม่ใช่ AI feature) | `POST /v1/financials` `{symbol:"AAPL"}` (เดี่ยว ไม่ใช่ batch) → `yfinance` `.income_stmt`/`.cashflow` server-side → `{symbol,currency,available,annual:[{year,revenue,netIncome,operatingCF,freeCF}]}` (เรียงใหม่→เก่า) หรือ `{symbol,available:false}` ถ้าไม่มีข้อมูล (กองทุน/ETF/ticker เล็กเกินไป/error — ทดสอบยืนยันแล้วว่า QQQ คืน `available:false` ถูกต้อง, AAPL/CRWV(เพิ่ง IPO)/PTT.BK ได้ข้อมูลจริงครบ 4-5 ปี); auth เหมือน `/v1/quote` (แค่ login, ไม่เช็ค subscription/quota) เพราะ yfinance ฟรี. **cache แรกในไฟล์ทั้งหมด** (`_financials_cache`, in-memory dict, TTL 24 ชม. — เหตุผล: `.income_stmt`/`.cashflow` หนักกว่า `.fast_info` มาก และงบเปลี่ยนแค่รายไตรมาส ไม่มีเหตุผลดึงใหม่ทุก request). ชื่อ row ใน yfinance DataFrame (`'Total Revenue'`/`'Net Income'`/`'Operating Cash Flow'`/`'Free Cash Flow'`) verify ตรงบน VM จริงแล้ว 2026-08-04 ก่อนเขียนโค้ด แต่เผื่อ fallback หลายชื่อไว้ (`_REVENUE_KEYS`/`_NET_INCOME_KEYS`/ฯลฯ ใน `ai_proxy.py`) เผื่อ ticker อื่น/เวอร์ชัน yfinance อื่นใช้ชื่อต่าง. **ใช้กับ UI:** ปุ่ม "ดูข้อมูลเชิงลึก" (`fa-chart-column`) ใน `_favStockCard` (ซ่อนถ้า `category==='fund'`) เปิด `openFavStockDetailModal(id)`/`closeFavStockDetailModal()` (คนละอันจาก `openFavStockModal` เดิมที่เป็น modal แก้ไข/เพิ่มหุ้น) — วาด 2 กราฟ Chart.js (กำไรขาดทุน: รายได้รวม vs กำไรสุทธิ, กระแสเงินสด: จากการดำเนินงาน vs อิสระ) ด้วย `_chartTickColor()`/`_chartGridColor()` เดิม + ฟังก์ชันใหม่ `_fmtFinNum()` ย่อเลขใหญ่เป็น K/M/B/T ให้อ่านง่าย; **ต้อง destroy chart ตอนปิด modal ด้วย** (`_fsdIncomeChart`/`_fsdCashflowChart` module-level, destroy+null ใน `closeFavStockDetailModal()` ก่อน `ov.remove()`) — เป็น precedent ใหม่ของไฟล์นี้ (ของเดิมทุกจุดที่ `.destroy()` ก่อน `new Chart()` เป็นเพราะ re-render container ซ้ำ ไม่ใช่ modal แบบ dynamic-create/remove); client cache แยกจากราคาหุ้น (`_favStockFinancialsCache`, TTL 12 ชม. — นานกว่าเพราะงบเปลี่ยนช้ากว่าราคามาก) ทดสอบ end-to-end บนบัญชี production จริงผ่านหมดทั้ง AAPL (กราฟจริง 2021-2025), QQQ (empty state ถูกต้อง), เปิด-ปิด-เปิดซ้ำไม่มี chart ซ้อน/ไม่มี console error |
