...
| I18N (เฟส 1, 2026-07-02) | **ระบบแปลภาษา TH/EN** — `I18N={th:{},en:{}}` + `t(key)` + `setLang()`/`toggleLang()` + `applyLang()`; เก็บที่ `localStorage.mm_lang` (ใช้ได้ก่อน login) + sync เข้า `DB.settings.lang` หลัง login; ปุ่มสลับภาษา `#lang-btn` (แสดงภาษาที่จะสลับไป) อยู่ข้าง theme toggle ในหน้า topbar; **ครอบคลุมแค่เมนู sidebar 19 รายการ + หัวข้อหน้า (`page-title`) + tagline** — เนื้อหาในแต่ละหน้า (ปุ่ม/label ภายใน), ข้อความ AI Maya, toast ฯลฯ **ยังเป็นภาษาไทยล้วน** (เฟสต่อไป); `PAGE_TITLE` แปลงเป็น `Proxy` ที่ route ผ่าน `t(page)` เพื่อให้โค้ดเดิม (`nav()`) ไม่ต้องแก้; เพิ่ม element ใหม่ต้องเติม `data-i18n="key"` แล้วเพิ่ม key เข้า `I18N.th`/`I18N.en` คู่กัน — **ต้องแก้ bug เดียวกับ push token**: `applyLang()` ต้องเรียกทั้งใน `enterApp()` (fresh login) และใน `checkLogin()` session-restore fast path (เช็ค `currentUser` ตรงๆ เพราะ patch `checkLogin` ไม่ทันการเรียกครั้งแรกที่ INIT block) |
