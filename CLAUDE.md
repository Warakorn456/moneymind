...
- **⚠️ Hermes บน OpenRouter ไม่เปิด `tools` API** — ทุกรุ่น (hermes-3/hermes-4) มี `supported_parameters` ไม่มี `'tools'` → ทำ OpenAI-style tool calling ไม่ได้แม้ตัวโมเดลจะเทรนมา; **ใช้โมเดลฟรีที่รองรับ tools แทน** — default = `openai/gpt-oss-120b:free` (ทดสอบแล้วตอบไทยดี ดึงข้อมูล Firestore จริง ฟรี $0); ทางเลือกอื่น `nvidia/nemotron-3-super-120b-a12b:free`; ถ้าจะใช้ Hermes จริงต้องเขียน native `<tool_call>` parser เพิ่ม (ยังไม่ทำ)
...
- **⚠️ Deploy caveat — ห้าม scp laptop `stock_bot.py` ทับ VM** — laptop version ล้าหลัง VM (ขาด `analyze_slip`/`analyze_voice`/`gemini_chat`); วิธี deploy Hermes: (1) scp `hermes_agent.py` + `mm_secrets.py` ขึ้น VM (เพิ่มอย่างเดียว ปลอดภัย) (2) เพิ่ม `MM_OPENROUTER_KEY` ใน `.env` บน VM (3) apply patch 2 จุดใน `stock_bot.py` ของ VM เอง (import + else-block fallback) — อย่าทับทั้งไฟล์
