...
| AI Usage | **หน้า `aiusage` (`renderAiUsage()`)** — มอนิเตอร์ค่า token/ค่า API ของ Gemini (ข้อมูล**จริง**จาก Cloud Monitoring project Best1, ครอบคลุมเว็บ Maya + บอท + ทุกสคริปต์): การ์ดสรุป (วันนี้/เดือนนี้/เฉลี่ย/คาดการณ์เดือน vs spend cap ฿100) + progress bar + กราฟแท่งรายวัน (Chart.js) + ตารางรายวัน. **แหล่งข้อมูล:** `gemini_usage_report.py --firestore` (cron 08:15 ทุกวัน) เขียนสรุปลง Firestore **field `aiUsage`** (stringValue JSON, แยกจาก `db` blob กัน race, PATCH updateMask=aiUsage ผ่าน SA token); เว็บอ่าน field นี้ใน `fsLoad`/`fsListen` → global `_aiUsage` → `renderAiUsage()`; chart instance `_aiUsageChart` |
