...
| Travel 3D Globe | **ลูกโลก 3 มิติหมุนได้** บนหน้า travel — `globe.gl@2.33.2` (Three.js bundled, lazy-load ผ่าน `_loadScript` ตอนเปิดหน้า) + texture earth-night จาก unpkg three-globe; พิมพ์ประเทศ/เมือง → `_geocode()` (Nominatim ฟรี ไม่ต้อง key, cache ใน `DB.travel.places`) → `_flyToPlace()` หมุนลูกโลกไปหา (`pointOfView`) → ปุ่ม **✓ เคยไปแล้ว / ★ อยากไป** (`travelMark`); หมุดเขียว=เคยไป เหลือง=อยากไป; auto-rotate toggle (`travelToggleSpin`); `_syncTravelPlacesFromTx()` geocode สถานที่จากรายการ tx อัตโนมัติ (ทีละ 1.1s กัน Nominatim rate-limit); **globe อยู่ใน card ระดับ page (persistent) แยกจาก `#travel-content` ที่ re-render** เพื่อไม่ทำลาย WebGL context; ข้อมูลเก็บใน `DB.travel.places=[{name,lat,lng,visited,label,fromTx}]` (sync localStorage+Firestore ปกติ) |
