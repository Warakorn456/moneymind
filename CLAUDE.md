...
| AI Inbox | **หน้า `inbox` (`renderInbox`)** — รวมรายการ **auto-import รอตรวจ** (สลิป Telegram/อีเมล/บอท) ที่ tags ตรง `slip-scan/quick-add/image-import/statement-import/voice-add/ai-agent/email-*` (หรือมี import/scan) + **ยังไม่ `reviewed`** + ใน 60 วันล่าสุด (`inboxItems`/`_isAutoTx`); การ์ดต่อใบ: แหล่งที่มา (`_inboxSrc`) + วันที่ + **⚠ ตรวจซ้ำ** (date+amount ตรงกัน) + dropdown เปลี่ยนหมวด (`setInboxCat`) + ยอด + ปุ่ม **ยืนยัน** (`confirmInbox` → ตั้ง `t.reviewed=true`)/แก้ (`editTx`)/ลบ (`delInboxTx`); ปุ่ม **ยืนยันทั้งหมด** (`confirmAllInbox`); **badge นับ** ที่ nav (`#inbox-badge`/`updateInboxBadge`, อัปเดตผ่าน monkey-patch `renderDash`); render สูงสุด 100 ใบ; nav `data-nav="inbox"` |
