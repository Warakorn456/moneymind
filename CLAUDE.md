...
- **🐛 หุ้นไทยไม่ใส่ `.BK` ต่อท้าย → ได้ข้อมูลบริษัทผิดตัวไปเลย ไม่ใช่แค่ error (เจอ+แก้ 2026-08-04 ดึกๆ)** — user รายงานว่า "ข้อมูลเชิงลึก" ของ SCB ไม่ขึ้น + KTB ขึ้นข้อมูล "ไม่ตรง" ต้องการธนาคารกรุงไทย; เช็ค `DB.favStocks` จริงพบว่า symbol ที่บันทึกไว้เป็น `KTB`/`SCB` เฉยๆ (ไม่มี `.BK`) — บน Yahoo Finance ticker `KTB` (ไม่มี suffix) ตรงกับ **Kontoor Brands, Inc.** (บริษัทเสื้อผ้าสหรัฐฯ เจ้าของแบรนด์ Wrangler/Lee, NYSE) คนละบริษัทกับธนาคารกรุงไทยโดยสิ้นเชิง ส่วน `SCB` เฉยๆ ไม่ resolve เป็นบริษัทจริงเลย (`shortName` คืนค่าประหลาดๆ ไม่ใช่ชื่อบริษัท) — ต้องมี `.BK` ต่อท้ายเสมอ (`KTB.BK`→Krung Thai Bank ถูกต้อง, `SCB.BK`→SCB X PCL ถูกต้องแต่งบการเงินไม่ครบเพราะปรับโครงสร้างเป็น SCBX ปี 2565 Yahoo ยังไม่มีข้อมูลย้อนหลังเต็ม — คนละปัญหากับเรื่อง suffix). **แก้ข้อมูลที่มีอยู่แล้วในบัญชีจริงให้ user โดยตรง** (แก้ symbol เป็น `KTB.BK`/`SCB.BK` ผ่าน `saveDB()`) + **แก้ต้นเหตุกันเกิดซ้ำ:** เปลี่ยน placeholder ช่อง symbol ในโมดัลเพิ่ม/แก้ไขหุ้น (`favstock_field_symbol_ph`) จาก `'เช่น NVDA, AAPL'` เป็น `'เช่น NVDA, AAPL, PTT.BK (หุ้นไทยต้องมี .BK ต่อท้ายเสมอ)'` (th+en) — **ยังไม่ได้ทำ validation บังคับ** (ยังกรอก `KTB` เฉยๆ ได้อยู่ ไม่มี warning ตอน save) แค่เพิ่ม hint ใน placeholder เท่านั้น ถ้าเจอ user พลาดแบบนี้อีกอาจต้องพิจารณา auto-append `.BK` ให้เมื่อเลือกหมวด "หุ้นไทย" หรือ validate symbol ผ่าน `/v1/quote`/`/v1/financials` ก่อน save จริง
