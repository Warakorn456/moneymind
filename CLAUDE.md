...
| Travel Globe gotcha | **Nominatim geocode ต้องเรียง `importance` แล้วเลือกตัวสูงสุด** — ค้นหา "ญี่ปุ่น" (accept-language=th) ผลแรกคือ "บ้านญี่ปุ่น อยุธยา ประเทศไทย" (importance 0.147) ไม่ใช่ประเทศญี่ปุ่น (0.938); fix: `limit=8` + `sort((a,b)=>b.importance-a.importance)[0]`. **ค้นหาใน UI ใช้ `_geocode(q,true)` (fresh) เสมอ** เลี่ยง cache พิกัดผิดเก่า + อัปเดตหมุดเดิมที่ผิดให้ถูกทันที; `_geocode` คืน `addresstype` → `_altForType()` กำหนด **zoom level** ของ MapLibre flyTo: country=4, state/province=6.5, city=10, town/village=12 → ค้น "ญี่ปุ่น" zoom กว้างเห็นทั้งประเทศ, "โอซาก้า" zoom ใกล้ระดับเมือง/ถนน |
